...
class FullyConnected(Layer):
    ...
    def backward(self, dLdY: np.ndarray) -> np.ndarray:
        ...
        dX = np.matmul(dLdZ, W.T)
        dW = np.matmul(X.T, dLdZ)
...
class Conv2D(Layer):
    ...
    def forward(self, X: np.ndarray) -> np.ndarray:
        ...
        out_ncols = ((in_cols - kernel_width + 2 * self.pad[1]) // self.stride) + 1

        Z = np.zeros((n_examples, out_nrows, out_ncols, out_channels))
        X_padded = np.pad(X, ((0, 0), (self.pad[0], self.pad[0]), 
                              (self.pad[1], self.pad[1]), (0, 0)), 'constant')
        
        for d1 in range(out_nrows):
            for d2 in range(out_ncols):
                for n in range(out_channels):
                    i_start, i_end = d1 * self.stride, d1 * self.stride + kernel_height
                    j_start, j_end =  d2 * self.stride, d2 * self.stride + kernel_width
                    curr_slice = X_padded[:, i_start:i_end, j_start:j_end, :]
                    Z[:, d1, d2, n] = np.sum(curr_slice * W[:, :, :, n], axis=(1, 2, 3)) + b[:, n]
        ...
        for d1 in range(out_nrows):
            for d2 in range(out_ncols):
                for n in range(out_channels):
                    ...
                    dW[:, :, :, n] += np.sum(curr_slice * dZ[:, d1:d1+1, d2:d2+1, None, n], axis=0)
                    dX_padded[:, i_start:i_end, j_start:j_end, :] += W[:, :, :, n] * dZ[:, d1:d1+1, d2:d2+1, None, n]
...
class Pool2D(Layer):
    ...
    def forward(self, X: np.ndarray) -> np.ndarray:
        ...
        X_padded = np.pad(X, ((0, 0), (self.pad[0], self.pad[0]), 
                              (self.pad[1], self.pad[1]), (0, 0)), 'constant')
        pool_shape = (n_examples, out_nrows, out_ncols, in_channels)
        X_pool = np.zeros(pool_shape)
        ...
        for d1 in range(out_nrows):
            for d2 in range(out_ncols):
                ...
                j_start, j_end =  d2 * self.stride, d2 * self.stride + kernel_width
                curr_slice = X_padded[:, i_start:i_end, j_start:j_end, :]
                X_pool[:, d1, d2, :] = self.pool_fn(curr_slice, axis=(1, 2))

        # cache any values required for backprop
        self.cache["out_rows"] = out_nrows
        self.cache["out_cols"] = out_ncols
    ...
    def backward(self, dLdY: np.ndarray) -> np.ndarray:
        ...
        out_nrows = self.cache["out_rows"]
        out_ncols = self.cache["out_cols"]
        ...
        for d1 in range(out_nrows):
            for d2 in range(out_ncols):
                ...
                if self.mode == 'max':
                    curr_slice = X_padded[:, i_start:i_end, j_start:j_end, :]
                    max_indices = np.argmax(curr_slice.reshape(n_examples, -1, in_channels), axis=1)
                    dX_curr = np.zeros(curr_slice.shape)
                    
                    n_idx, c_idx = np.indices((n_examples, in_channels))
                    max_rows, max_cols = np.unravel_index(max_indices, (kernel_height, kernel_width))
                    
                    dX_curr[n_idx, max_rows, max_cols, c_idx] = dLdY[:, d1, d2, :]
                    dX_padded[:, i_start:i_end, j_start:j_end, :] += dX_curr

                elif self.mode == "average":
                    dX_padded[:, i_start:i_end, j_start:j_end, :] += dLdY[:, d1:d1+1, d2:d2+1, :] / (kernel_height * kernel_width)
